...
def train(model, loss_func, metric_func,
              ...
              start_epoch: int = 0,
              print_freq: int = 20,
              model_save_path='./data_largebatch_gnot/checkpoints/',
...
if __name__ == "__main__":
    ...
    if args.resume:
        ckpt = load_checkpoint('./data_largebatch_gnot/checkpoints/')
    ...
    if args.use_tb:
        writer_path =  './data_largebatch_gnot/logs/' + path_prefix
    ...
    torch.save(checkpoint, os.path.join('./data_largebatch_gnot/checkpoints/{}'.format(model_path)))
    model.eval()
